...
exports.insertData = (response, searchStatement, insertStatement, data) => {
    koneksi.query(searchStatement, data.course_category_id, (err, rows, field) => {
        if (err) {
            return response.status(500).json({ message: 'Ada kesalahan', error: err });
        }
        ...
        if (rows.length) {
            ...
            return response.status(404).json({ message: 'Kategori kursus tidak ditemukan!', success: false });
        }
    });
};
