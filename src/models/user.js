...
exports.getDataById = (response, searchStatement, id) => {
  koneksi.query(searchStatement, [id], (err, rows, field) => {
    ...
    if (rows.length) {
      ...
      return response.status(404).json({ message: "Peserta tidak ditemukan!", success: false });
    }
  });
};
...
exports.deleteData = (response, searchStatement, deleteStatement, id) => {
  koneksi.query(searchStatement, id, (err, rows, field) => {
    ...
    if (rows.length) {
      ...
      return response.status(404).json({ success: false, message: "Peserta tidak ditemukan!" });
    }
  });
};
